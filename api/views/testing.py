...
def view():
    ...
    if addOrder_Form.validate_on_submit():
            order_to_add = Order(
                 buyer_id = addOrder_Form.buyer_id.data,
                 seller_id = addOrder_Form.seller_id.data,
                 total_price = addOrder_Form.total_price.data,
                 status = addOrder_Form.status.data,
                 order_date = addOrder_Form.order_date.data
            )
            database.db.session.add(order_to_add)
            database.db.session.commit()
            return flask.render_template("home.html",dishes = dishes, users = users, orders= orders,current_user=current_user,add_Dish_form=add_Dish_form,deleteDish_form=deleteDish_form,add_user_form=add_user_form,addOrder_Form=addOrder_Form)
